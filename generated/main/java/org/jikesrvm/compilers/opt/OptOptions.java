 ...
 * <p>
 * Note: Boolean options are defined in /home/csgrad/pnallamo/605/Test/Project605/rvm/src-generated/options/BooleanOptions.opt.dat /home/csgrad/pnallamo/605/Test/Project605/rvm/src-generated/options/SharedBooleanOptions.dat
 *       All other options are defined in /home/csgrad/pnallamo/605/Test/Project605/rvm/src-generated/options/ValueOptions.opt.dat /home/csgrad/pnallamo/605/Test/Project605/rvm/src-generated/options/SharedValueOptions.dat
...
public class OptOptions implements Cloneable {
  ...
  private void printOptionsHeader() {
    VM.sysWriteln("Current value of options at optimization level ",OPTIMIZATION_LEVEL, ":");
  }
 ...
 */
   ...
   public boolean PRINT_VISUALIZATION           = false;
   /** When dumping the IR without using visualization, additionally dump the output for each phase to a separate file */
   public boolean PRINT_PHASES_TO_FILES         = false;
   ...
   public boolean PRINT_MACHINECODE             = false;
    ...
    public String VISUALIZE_IR_DIRECTORY         = "";
    ...
    *
    * @return {@code true} when PROFILE_COUNTERS_FREQ is set to PROFILE_FREQUENCY_STRATEGY,
    ...
    *
    * @return {@code true} when PROFILE_STATIC_FREQ is set to PROFILE_FREQUENCY_STRATEGY,
    *   {@code false} otherwise
    */
    ...
    *
    * @return {@code true} when PROFILE_DUMB_FREQ is set to PROFILE_FREQUENCY_STRATEGY,
    *   {@code false} otherwise
    */
    ...
    *
    * @return {@code true} when PROFILE_INVERSE_COUNTERS_FREQ is set to PROFILE_FREQUENCY_STRATEGY,
    *   {@code false} otherwise
    */
    ...
    *
    * @return {@code true} when REGALLOC_SIMPLE_SPILL_COST is set to REGALLOC_SPILL_COST_ESTIMATE,
    *   {@code false} otherwise
    */
    ...
    *
    * @return {@code true} when REGALLOC_BRAINDEAD_SPILL_COST is set to REGALLOC_SPILL_COST_ESTIMATE,
    *   {@code false} otherwise
    */
    ...
    *
    * @return {@code true} when REGALLOC_BLOCK_COUNT_SPILL_COST is set to REGALLOC_SPILL_COST_ESTIMATE,
    *   {@code false} otherwise
    */
    ...
    *
    * @return {@code true} when INLINE_GUARD_METHOD_TEST is set to INLINE_GUARD_KIND,
    *   {@code false} otherwise
    */
    ...
    *
    * @return {@code true} when INLINE_GUARD_CLASS_TEST is set to INLINE_GUARD_KIND,
    *   {@code false} otherwise
    */
    ...
    *
    * @return {@code true} when INLINE_GUARD_CODE_PATCH is set to INLINE_GUARD_KIND,
    *   {@code false} otherwise
    */
    ...
    *
    * @return an iterator over the items in DRIVER_EXCLUDE. The iterator
    *   may be empty.
    */
    ...
    *
    * @return an iterator over the items in METHOD_TO_PRINT. The iterator
    *   may be empty.
    */
  ...
  public boolean processAsOption(String prefix, String arg) {
    ...
    if (name.equals("print_visualization")) {
      if (value.equals("true")) {
    ...
    if (name.equals("print_phases_to_files")) {
      if (value.equals("true")) {
        PRINT_PHASES_TO_FILES = true;
        return true;
      } else if (value.equals("false")) {
        PRINT_PHASES_TO_FILES = false;
        return true;
      } else
        return false;
    }
    ...
    if (name.equals("visualize_ir_directory")) {
       VISUALIZE_IR_DIRECTORY = value;
       return true;
     }
  ...
  public static void printHelp(String prefix) {
    ...
    VM.sysWriteln("Option                        OptLevel Description");
    VM.sysWriteln("focus_effort                           Focus compilation effort based on frequency profile data");
    VM.sysWriteln("reads_kill                             Should we constrain optimizations by enforcing reads-kill?");
    VM.sysWriteln("field_analysis                 0       Eagerly compute method summaries for flow-insensitive field analysis");
    VM.sysWriteln("inline                         0       Inline statically resolvable calls");
    VM.sysWriteln("inline_guarded                 0       Guarded inlining of non-final virtual calls");
    VM.sysWriteln("inline_guarded_interfaces      0       Speculatively inline non-final interface calls");
    VM.sysWriteln("inline_preex                   0       Pre-existence based inlining");
    VM.sysWriteln("simplify_integer_ops                   Simplify operations on integers");
    VM.sysWriteln("simplify_long_ops                      Simplify operations on longs");
    VM.sysWriteln("simplify_float_ops                     Simplify operations on floats");
    VM.sysWriteln("simplify_double_ops                    Simplify operations on floats");
    VM.sysWriteln("simplify_ref_ops                       Simplify operations on references");
    VM.sysWriteln("simplify_tib_ops                       Simplify operations on TIBs");
    VM.sysWriteln("simplify_field_ops                     Simplify operations on fields");
    VM.sysWriteln("simplify_chase_final_fields            Chase final fields avoiding loads at runtime");
    VM.sysWriteln("local_constant_prop            0       Perform local constant propagation");
    VM.sysWriteln("local_copy_prop                0       Perform local copy propagation");
    VM.sysWriteln("local_cse                      0       Perform local common subexpression elimination");
    VM.sysWriteln("local_expression_folding       3       Should we try to fold expressions with constants locally?");
    VM.sysWriteln("control_static_splitting       1       CFG splitting to create hot traces based on static heuristics");
    VM.sysWriteln("control_unwhile                3       Turn whiles into untils");
    VM.sysWriteln("escape_simple_ipa                      Eagerly compute method summaries for simple escape analysis");
    VM.sysWriteln("escape_scalar_replace_aggregates 1       If possible turn aggregates (objects) into variable definition/uses");
    VM.sysWriteln("escape_monitor_removal         1       Try to remove unnecessary monitor operations");
    VM.sysWriteln("escape_invokee_thread_local            Compile the method assuming the invokee is thread-local. Cannot be properly set on command line.");
    VM.sysWriteln("ssa                            3       Should SSA form be constructed on the HIR?");
    VM.sysWriteln("ssa_expression_folding         3       Should we try to fold expressions with constants in SSA form?");
    VM.sysWriteln("ssa_redundant_branch_elimination 3       Eliminate redundant conditional branches");
    VM.sysWriteln("ssa_licm_ignore_pei                    Assume PEIs do not throw or state is not observable");
    VM.sysWriteln("ssa_load_elimination           3       Should we perform redundant load elimination during SSA pass?");
    VM.sysWriteln("ssa_coalesce_after             3       Should we coalesce move instructions after leaving SSA?");
    VM.sysWriteln("ssa_loop_versioning                    Create copies of loops where runtime exceptions are checked prior to entry");
    VM.sysWriteln("ssa_live_range_splitting               Split live ranges using LIR SSA pass?");
    VM.sysWriteln("ssa_gcp                        3       Perform global code placement");
    VM.sysWriteln("ssa_gcse                       3       Perform global code placement");
    VM.sysWriteln("ssa_splitblock_to_avoid_rename         When leaving SSA create blocks to avoid renaming variables");
    VM.sysWriteln("ssa_splitblock_for_local_live          When leaving SSA create blocks for local liveness");
    VM.sysWriteln("ssa_splitblock_into_infrequent         When leaving SSA create blocks to avoid adding code to frequently executed blocks");
    VM.sysWriteln("reorder_code                   0       Reorder basic blocks for improved locality and branch prediction");
    VM.sysWriteln("reorder_code_ph                1       Reorder basic blocks using Pettis and Hansen Algo2");
    VM.sysWriteln("h2l_inline_new                 0       Inline allocation of scalars and arrays");
    VM.sysWriteln("h2l_inline_write_barrier       1       Inline write barriers for generational collectors");
    VM.sysWriteln("h2l_inline_primitive_write_barrier 1       Inline primitive write barriers for certain collectors");
    VM.sysWriteln("h2l_no_callee_exceptions               Assert that any callee of this compiled method will not throw exceptions. Cannot be properly set on command line.");
    VM.sysWriteln("h2l_call_via_jtoc                      Plant virtual calls via the JTOC rather than from the tib of anobject when possible");
    VM.sysWriteln("l2m_handler_liveness           2       Store liveness for handlers to improve dependence graph at PEIs");
    VM.sysWriteln("regalloc_coalesce_moves        0       Attempt to coalesce to eliminate register moves?");
    VM.sysWriteln("regalloc_coalesce_spills       0       Attempt to coalesce stack locations?");
    VM.sysWriteln("adaptive_instrumentation_sampling         Perform code transformation to sample instrumentation code.");
    VM.sysWriteln("adaptive_no_duplication                When performing inst. sampling, should it be done without duplicating code?");
    VM.sysWriteln("adaptive_processor_specific_counter         Should there be one CBS counter per processor for SMP performance?");
    VM.sysWriteln("adaptive_remove_yp_from_checking         Should yieldpoints be removed from the checking code (requires finite sample interval)");
    VM.sysWriteln("osr_guarded_inlining           1       Insert OSR point at off branch of guarded inlining?");
    VM.sysWriteln("osr_inline_policy              1       Use OSR knowledge to drive more aggressive inlining?");
    VM.sysWriteln("print_static_stats                     Print out compile-time statistics for basic blocks?");
    VM.sysWriteln("print_phases                           Print short message for each compilation phase");
    VM.sysWriteln("print_all_ir                           Dump the IR after each compiler phase");
    VM.sysWriteln("print_detailed_inline_report           Print detailed report of compile-time inlining decisions");
    VM.sysWriteln("print_inline_report                    Print detailed report of compile-time inlining decisions");
    VM.sysWriteln("print_dom                              Print dominators");
    VM.sysWriteln("print_pdom                             Print post-dominators");
    VM.sysWriteln("print_ssa                              Print SSA form");
    VM.sysWriteln("print_dg_burs                          Print dependence graph before burs");
    VM.sysWriteln("print_visualization                    When dumping the IR, generate a visualization in a file instead");
    VM.sysWriteln("print_phases_to_files                  When dumping the IR without using visualization, additionally dump the output for each phase to a separate file");
    VM.sysWriteln("print_high                             Print IR after initial generation");
    VM.sysWriteln("print_final_hir                        Print IR just before conversion to LIR");
    VM.sysWriteln("print_low                              Print IR after conversion to LIR");
    VM.sysWriteln("print_final_lir                        Print IR just before conversion to MIR");
    VM.sysWriteln("print_mir                              Print IR after conversion to MIR");
    VM.sysWriteln("print_final_mir                        Print IR just before conversion to machine code");
    VM.sysWriteln("print_cfg                              Print control flow graph too when IR is printed");
    VM.sysWriteln("print_regalloc                         Print IR before and after register allocation");
    VM.sysWriteln("print_calling_conventions              Print IR after expanding calling conventions");
    VM.sysWriteln("print_gc_maps                          Print the garbage collection maps");
    VM.sysWriteln("debug_cgd                              Enable debugging support for final assembly");
    VM.sysWriteln("debug_instru_sampling                  Enable debugging statements for instrumentation sampling");
    VM.sysWriteln("debug_instru_sampling_detail           Enable detailed debugging statements for instrumentation sampling");
    VM.sysWriteln("debug_gcp                              Perform noisy global code placement");
    VM.sysWriteln("verbose                                Print method name at start of compilation");
    VM.sysWriteln("mc                                     Print final machine code");
    VM.sysWriteln();
    VM.sysWrite("Value Options ("+prefix+"<option>=<value>)");
    VM.sysWriteln("Option                         Type    Description");
    VM.sysWriteln("print_ir_level                 int     Only print IR compiled above this level");
    VM.sysWriteln("visualize_ir_directory         String  If visualization is enabled, the directory to write visualization files to");
    VM.sysWriteln("profile_edge_count_input_file  String  Input file of edge counter profile data");
    VM.sysWriteln("profile_infrequent_threshold   float   Cumulative threshold which defines the set of infrequent basic blocks");
    VM.sysWriteln("profile_cbs_hotness            double  Threshold at which a conditional branch is considered to be skewed");
    VM.sysWriteln("escape_max_array_size          int     Maximum size of array to replaced with registers by simple escape analysis");
    VM.sysWriteln("ssa_load_elimination_rounds    int     How many rounds of redundant load elimination will we attempt?");
    VM.sysWriteln("l2m_max_block_size             int     Maximum size of block for BURS in real instructions, larger blocks are split.");
    VM.sysWriteln("regalloc_simple_spill_cost_move_factor double  spill penalty for move instructions");
    VM.sysWriteln("regalloc_simple_spill_cost_memory_operand_factor double  spill penalty for registers used in memory operands");
    VM.sysWriteln("control_tableswitch_cutoff     int     If a tableswitch comprises this many or fewer comparisons convert it into multiple if-then-else style branches");
    VM.sysWriteln("control_cond_move_cutoff       int     How many extra instructions will we insert in order to remove a conditional branch?");
    VM.sysWriteln("control_unroll_log             int     Unroll loops. Duplicates the loop body 2^n times.");
    VM.sysWriteln("control_static_splitting_max_cost int     Upper bound on the number of instructions duplicated per block when trying to create hot traces with static splitting");
    VM.sysWriteln("control_well_predicted_cutoff  double  Don't replace branches with conditional moves if they are outside of the range of 0.5 +- this value");
    VM.sysWriteln("inline_max_target_size         int     Static inlining heuristic: Upper bound on callee size");
    VM.sysWriteln("inline_max_inline_depth        int     Static inlining heuristic: Upper bound on depth of inlining");
    VM.sysWriteln("inline_max_always_inline_target_size int     Static inlining heuristic: Always inline callees of this size or smaller");
    VM.sysWriteln("inline_massive_method_size     int     Static inlining heuristic: If root method is already this big, then only inline trivial methods");
    VM.sysWriteln("inline_max_arg_bonus           double  Maximum bonus for reducing the perceived size of a method during inlining.");
    VM.sysWriteln("inline_precise_reg_array_arg_bonus double  Bonus given to inlining methods that are passed a register of a known precise type.");
    VM.sysWriteln("inline_declared_aastored_array_arg_bonus double  Bonus given when there's potential to optimize checkstore portion of aastore bytecode on parameter");
    VM.sysWriteln("inline_precise_reg_class_arg_bonus double  Bonus given to inlining methods that are passed a register of a known precise type.");
    VM.sysWriteln("inline_extant_reg_class_arg_bonus double  Bonus given to inlining methods that are passed a register that's known not to be null.");
    VM.sysWriteln("inline_int_const_arg_bonus     double  Bonus given to inlining methods that are passed an int constant argument");
    VM.sysWriteln("inline_null_const_arg_bonus    double  Bonus given to inlining methods that are passed a null constant argument");
    VM.sysWriteln("inline_object_const_arg_bonus  double  Bonus given to inlining methods that are passed an object constant argument");
    VM.sysWriteln("inline_call_depth_cost         double  As we inline deeper nested methods what cost (or bonus) do we wish to give to deter (or encourage) nesting of deeper methods?");
    VM.sysWriteln("inline_ai_max_target_size      int     Adaptive inlining heuristic: Upper bound on callee size");
    VM.sysWriteln("inline_ai_min_callsite_fraction double  Adaptive inlining heuristc: Minimum fraction of callsite distribution for guarded inlining of a callee");
    VM.sysWrite("\nSelection Options (set option to one of an enumeration of possible values)\n");
    VM.sysWriteln("\t\tHow to compute block and edge frequencies?");
    ...
    VM.sysWrite("inverse ");
    VM.sysWriteln();
    VM.sysWriteln("\t\tSelection of spilling heuristic");
    VM.sysWrite("regalloc_spill_cost_estimate  ");
    VM.sysWrite("simple ");
    VM.sysWrite("brainDead ");
    VM.sysWrite("blockCount ");
    VM.sysWriteln();
    VM.sysWriteln("\t\tSelection of guard mechanism for inlined virtual calls that cannot be statically bound");
    ...
    VM.sysWriteln("driver_exclude                 Exclude methods from being opt compiled");
    VM.sysWriteln("method_to_print                Only apply print options against methods whose name contains this string");
  ...
  public String toString() {
    ...
    result.append("\tprint_visualization            = ").append(PRINT_VISUALIZATION).append("\n");
    result.append("\tprint_phases_to_files          = ").append(PRINT_PHASES_TO_FILES).append("\n");
    ...
    result.append("\tvisualize_ir_directory         = ").append(VISUALIZE_IR_DIRECTORY).append("\n");
    ...
    if (PROFILE_FREQUENCY_STRATEGY == PROFILE_COUNTERS_FREQ)
       result.append("\tprofile_frequency_strategy     = PROFILE_COUNTERS_FREQ\n");
    if (PROFILE_FREQUENCY_STRATEGY == PROFILE_STATIC_FREQ)
       result.append("\tprofile_frequency_strategy     = PROFILE_STATIC_FREQ\n");
    if (PROFILE_FREQUENCY_STRATEGY == PROFILE_DUMB_FREQ)
       result.append("\tprofile_frequency_strategy     = PROFILE_DUMB_FREQ\n");
    if (PROFILE_FREQUENCY_STRATEGY == PROFILE_INVERSE_COUNTERS_FREQ)
       result.append("\tprofile_frequency_strategy     = PROFILE_INVERSE_COUNTERS_FREQ\n");
    if (REGALLOC_SPILL_COST_ESTIMATE == REGALLOC_SIMPLE_SPILL_COST)
       result.append("\tregalloc_spill_cost_estimate   = REGALLOC_SIMPLE_SPILL_COST\n");
    if (REGALLOC_SPILL_COST_ESTIMATE == REGALLOC_BRAINDEAD_SPILL_COST)
       result.append("\tregalloc_spill_cost_estimate   = REGALLOC_BRAINDEAD_SPILL_COST\n");
    if (REGALLOC_SPILL_COST_ESTIMATE == REGALLOC_BLOCK_COUNT_SPILL_COST)
       result.append("\tregalloc_spill_cost_estimate   = REGALLOC_BLOCK_COUNT_SPILL_COST\n");
    if (INLINE_GUARD_KIND == INLINE_GUARD_METHOD_TEST)
       result.append("\tinline_guard_kind              = INLINE_GUARD_METHOD_TEST\n");
    if (INLINE_GUARD_KIND == INLINE_GUARD_CLASS_TEST)
       result.append("\tinline_guard_kind              = INLINE_GUARD_CLASS_TEST\n");
    if (INLINE_GUARD_KIND == INLINE_GUARD_CODE_PATCH)
       result.append("\tinline_guard_kind              = INLINE_GUARD_CODE_PATCH\n");
    {
  ...
  public void printOptions() {
    ...
    VM.sysWriteln("\tprint_visualization            = ",PRINT_VISUALIZATION);
    VM.sysWriteln("\tprint_phases_to_files          = ",PRINT_PHASES_TO_FILES);
    ...
    VM.sysWriteln("\tvisualize_ir_directory         = ",VISUALIZE_IR_DIRECTORY);
...
// END CODE GENERATED FROM MasterOptions.template
  ...
  private static void instancePrintHelpHeader(String prefix) {
    ...
    VM.sysWriteln(prefix+"[help]\t\t\tPrint brief description of opt compiler's command-line arguments");
    VM.sysWriteln(prefix+"printOptions\t\tPrint the current values of opt compiler options");
    if (prefix.indexOf("irc")!=-1 || prefix.indexOf("bc")!=-1 || prefix.indexOf("eoc")!=-1) {
      VM.sysWriteln(prefix+"O0\t\t\tSelect optimization level 0, minimal optimizations");
      VM.sysWriteln(prefix+"O1\t\t\tSelect optimization level 1, modest optimizations");
      VM.sysWriteln(prefix+"O2\t\t\tSelect optimization level 2");
    }
    VM.sysWriteln();
  }
   ...
   *
   * @param level optimization level, must be at least 0
   */
